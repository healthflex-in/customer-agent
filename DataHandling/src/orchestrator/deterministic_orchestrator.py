...
class DeterministicOrchestrator:
    ...
    def get_next_question(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        for question in all_questions:
            ...
            if q_id and q_id in completed_questions:
                continue
        ...
        # Sort by priority (higher first); created_at may be a datetime or numeric timestamp
        def _created_ts(q):
            val = q.get("metadata", {}).get("created_at", None)
        ...
        candidate_questions.sort(
            key=lambda q: (q.get("priority", 0), -_created_ts(q)),
            reverse=True,
        )
